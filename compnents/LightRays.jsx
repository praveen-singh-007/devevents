...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (mouseInfluence > 0.0) {
    ...
    vec2 mouseDirection =
      normalize(mouseScreenPos - rayPos);

    finalRayDir =
      normalize(
        mix(rayDir, mouseDirection, mouseInfluence)
      );
  }
  ...
  if (noiseAmount > 0.0) {
    float n =
      noise(coord * 0.01 + iTime * 0.1);

    fragColor.rgb *=
      (1.0 - noiseAmount + noiseAmount * n);
  }

  float brightness =
    1.0 - (coord.y / iResolution.y);
  ...
  if (saturation != 1.0) {
    ...
    fragColor.rgb =
      mix(vec3(gray), fragColor.rgb, saturation);
  }
